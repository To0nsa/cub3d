...
{
	if (ray->map.x < 0 || ray->map.x >= game->map->size.x || ray->map.y < 0 ||
	    ray->map.y >= game->map->size.y)
	{
		ray->hit = 1;
		return (true);
	}
                                              ...
                                              t_door* door)
{
	...
	if (ray->side == 0)
		door_x =
		    game->player.pos.y +
		    ((ray->map.x - game->player.pos.x + (1 - ray->step_dir.x) / 2.0) /
		     ray->dir.x) *
		        ray->dir.y;
	else
		door_x =
		    game->player.pos.x +
		    ((ray->map.y - game->player.pos.y + (1 - ray->step_dir.y) / 2.0) /
		     ray->dir.y) *
		        ray->dir.x;
	door_x -= floor(door_x);
