 ...
 */
static inline int calc_tex_pixel_x(int stripe_x, int screen_x, int sprite_width,
                                   int texture_width)
{
	int relative_x;
...
static inline unsigned int get_tex_color(t_texture* tex, int x, int y)
{
	...
	if (x < 0 || x >= tex->size.x || y < 0 || y >= tex->size.y)
		return (42);
	pixel_ptr =
	    (unsigned int*) (tex->addr + y * tex->line_size + x * (tex->bpp >> 3));
	color = *pixel_ptr;
...
void draw_sprite_column(t_game* game, t_sprite_draw* data, double* z_buffer)
{
	...
	{
		data->tex_pixel.y =
		    get_texture_y(screen_pos.y, data->height, data->texture->size.y);
		color =
		    get_tex_color(data->texture, data->tex_pixel.x, data->tex_pixel.y);
