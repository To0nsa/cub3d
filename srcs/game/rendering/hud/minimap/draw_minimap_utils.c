/**
 * @file draw_minimap_utils.c
 * @author nlouis
 * @date 2025/04/10
 * @brief Utility functions for minimap layout and geometry.
 *
 * @details
 * This file provides low-level helper functions used to calculate
 * screen-space coordinates and layout for rendering the minimap HUD.
 * These utilities handle tile sizing, coordinate transformations, and
 * geometric inclusion tests (like circle overlap).
 *
 * These functions are typically used by `draw_minimap()` to ensure
 * proper scaling and placement of map elements.
 *
 * Functions included:
 * - `get_tile_size()`: Computes a suitable tile size for current map dimensions
 * - `get_minimap_position()`: Converts map grid positions to screen coordinates
 * - `is_point_in_circle()`: Used for drawing circular UI elements like the
 * player
 *
 * @ingroup hud
 */
