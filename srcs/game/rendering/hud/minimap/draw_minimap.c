 ...
 * @author nlouis
 * @date 2025/04/10
 * @brief Minimap rendering logic for the HUD.
 *
 * @details
 * This file implements the rendering of the in-game minimap, including:
 * - Drawing wall tiles as square blocks
 * - Rendering the player icon as a circular dot
 * - Handling pixel placement directly within the image buffer
 *
 * It combines map matrix analysis with screen-space transformations to
 * visually display the surrounding area to the player. The minimap is
 * only drawn if `game->minimap_visible` is enabled.
 *
 * Key components:
 * - `draw_minimap()` is the public entry point
 * - `draw_minimap_walls()` scans the matrix and renders map walls
 * - `draw_player_on_minimap()` draws the player dot in screen-space
 * - `draw_square_block()` performs raw square rendering
 *
 * This HUD module provides an important spatial reference during gameplay.
 *
 * @ingroup hud
 */
