/**
 * @file draw_npc_dialogue.c
 * @author nlouis
 * @date 2025/04/10
 * @brief Renders dialogue lines for speaking NPCs in the HUD.
 *
 * @details
 * This file implements the logic for displaying interactive dialogue
 * between the player and NPCs within the HUD. Dialogue lines are rendered
 * in a centered dialogue box at the bottom of the screen when an NPC is
 * in the `SPEAK` state.
 *
 * Rendering steps include:
 * - Validating the NPC’s dialogue state (`should_draw_npc_dialogue`)
 * - Fetching the current dialogue line (`get_current_dialogue_line`)
 * - Drawing the dialogue box and text to the screen (`render_npc_dialogue`)
 * - Looping over all NPCs and conditionally rendering (`draw_npc_dialogue`)
 *
 * Text is rendered using MiniLibX’s `mlx_string_put()`, and the background
 * uses a preloaded dialogue box texture.
 *
 * @ingroup hud
 */
...
static char* get_current_dialogue_line(t_npc* npc)
{
	if (npc->dialogue.current_line >=
		...
		npc->dialogue.current_line = 0;
	return (npc->dialogue
	            .dialogues[npc->dialogue.phase][npc->dialogue.current_line]);
}
