/**
 * @file reset_astar_struct.c
 * @author nlouis
 * @date 2025/04/10
 * @brief Cleanup logic for A* pathfinding memory structures.
 *
 * @details
 * This file provides functions to deallocate all dynamic memory used during
 * an A* pathfinding operation. It ensures that:
 * - The temporary linked list of `t_node` structures is fully freed
 * - The closed list (binary min-heap of visited nodes) is deallocated
 * - The open list (2D visited state grid) is also released
 *
 * These operations are crucial for preventing memory leaks when an NPC
 * performs multiple consecutive pathfinding calls.
 *
 * The `reset_astar_struct()` function serves as the public interface, while
 * `free_astar_nodes()` and `free_priority_queue()` are internal helpers.
 *
 * @note
 * This does **not** deallocate the `t_astar` struct itself — only its contents.
 * Safe to call before or after any pathfinding cycle.
 *
 * @ingroup AStar
 */
