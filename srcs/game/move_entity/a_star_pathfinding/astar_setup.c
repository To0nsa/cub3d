 ...
 * @author nlouis
 * @date 2025/04/10
 * @brief Initialization and setup logic for the A* pathfinding system.
 *
 * @details
 * This file contains the setup routines for configuring an A* search session
 * for NPCs in the game world. It prepares the necessary data structures and
 * resets any previous search state before a new pathfinding attempt.
 *
 * Specifically, it provides:
 * - Creation of the closed list (min-heap) for visited nodes
 * - Allocation and zero-initialization of the open list (2D presence grid)
 * - Construction of the start node with correct cost values
 * - Complete setup of the `t_astar` structure for a fresh pathfinding run
 *
 * These routines are critical for isolating and preparing each A* operation
 * independently, ensuring no leftover state from previous paths interferes
 * with the current one.
 *
 * @note
 * All allocations use the custom allocator system via `x_malloc` and `x_calloc`.
 * Errors during allocation will trigger `error()` and abort the program.
 *
 * @ingroup AStar
 */
...
static t_closed_list* create_closed_list(t_game* game)
{
	...
	closed_list->nodes =
	    x_malloc(game, sizeof(t_node*) * closed_list->capacity);
	closed_list->size = 0;
	return (closed_list);
}
...
static int** create_open_list(t_game* game)
{
	...
	{
		open_list[y] = (int*) x_calloc(game, game->map->size.x, sizeof(int));
		if (!open_list[y])
		{
			while (--y >= 0)
				free(open_list[y]);
