/**
 * @file astar_closed_list.c
 * @author nlouis
 * @date 2025/04/10
 * @brief Min-heap implementation for the A* closed list system.
 *
 * @details
 * This file implements the binary heap (min-heap) logic used to manage
 * the closed list in the A* pathfinding algorithm. Each node in the
 * closed list represents a previously visited cell on the map.
 *
 * The heap allows fast extraction of the node with the lowest `f_cost`
 * — a key part of the A* algorithm — and supports efficient insertions
 * through `heapify_up()` and removals via `heapify_down()`.
 *
 * All allocations are handled using the custom memory system (see `x_malloc`
 * and `x_realloc`) provided by the game context.
 *
 * @note
 * This implementation assumes the closed list is always a complete binary tree
 * and uses array-based indexing for parent-child relationships.
 *
 * @ingroup AStar
 */
 ...
 * @ingroup AStar
 */
void closed_list_insert(t_closed_list* closed_list, t_node* node, t_game* game)
{
	if (closed_list->size >= closed_list->capacity)
	{
		closed_list->capacity *= 2;
		closed_list->nodes =
		    (t_node**) x_realloc(game, closed_list->nodes,
